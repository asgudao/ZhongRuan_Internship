...
import {userPage,userDeleteById} from '@/apis/user'
...
const searchUserPage= async (pageNo,pageSize,username)=>{
    const data = {
        pageNo,
        pageSize,
        username
    }
    //userPage(data).then((res)=>tableData.value=res.data.list)
    const result  = await userPage(data)
...
onMounted(()=>{
    ...
    searchUserPage(pageNo.value,pageSize.value,username.value)
})
...
const handleSizeChange=(value)=>{
    ...
    localStorage.setItem('userPageSize', value.toString());
    searchUserPage(pageNo.value,value,username.value)
}
//页码发生改变回调函数
const handleCurrentChange=(value)=>{
    console.log(`pageNo=${value}`)
    searchUserPage(value,pageSize.value,username.value)
}
//跳转到新增页面
const handleAdd=()=>{
    router.push({
...
<template>
    ...
    <div class="div1">
        ...
        >
            ...
            <!-- <el-table-column prop="avatarPath" label="头像" width="180" /> -->
            <el-table-column label="头像" width="180">
                <template #default="scope">
                    <el-image style="width: 30%;" :src="scope.row.avatarPath" fit="fill" ></el-image>
                </template>
            </el-table-column>

...
import {userUpdate,userDetail} from '@/apis/user'
...
onMounted(async ()=>{
    ...
    //todo 请求后端api 
    const user = (await userDetail(id)).data
    console.log(user)
    dataForm.id=user.id
    dataForm.username=user.username
    dataForm.avatarPath=user.avatarPath
    dataForm.email=user.email
    dataForm.telephone=user.telephone
    dataForm.qq=user.qq
    dataForm.wechat=user.wechat
})

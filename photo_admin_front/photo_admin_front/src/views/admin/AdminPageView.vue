...
import {adminPage,adminDeleteById} from '@/apis/admin'
...
const searchAdminPage= async (pageNo,pageSize,username)=>{
    const data = {
        pageNo,
        pageSize,
        username
    }
    //adminPage(data).then((res)=>tableData.value=res.data.list)
    const result  = await adminPage(data)
...
onMounted(()=>{
    ...
    searchAdminPage(pageNo.value,pageSize.value,username.value)
})
...
const handleSizeChange=(value)=>{
    ...
    localStorage.setItem('adminPageSize', value.toString());
    searchAdminPage(pageNo.value,value,username.value)
}
//页码发生改变回调函数
const handleCurrentChange=(value)=>{
    console.log(`pageNo=${value}`)
    searchAdminPage(value,pageSize.value,username.value)
}
//跳转到新增页面
const handleAdd=()=>{
    router.push({
